function TriggerOnView(defaultOptions = null, passedOptions = null)
{
    ...
    if(Array.isArray(passedOptions)){
        for(key in passedOptions)
        {
            ...
            // If element not set then return as there is nothing to do
            if(passedOptions[key].element == undefined)
            {
                return;
            }
    ...
    // Setup triggered and trigger buffer
    var triggerd = false;
    ...
    function checkTrigger()
    {
        ...
        var topOfWindow = (document.documentElement && document.documentElement.scrollTop) || (document.body.scrollTop) || (document.scrollingElement.scrollTop); // scrollTop
        var bottomOfWindow = topOfWindow + windowHeight;
        
        // Get the top and bottom Y position of the tigger element
        var triggerElementTop = options.trigger.getBoundingClientRect().top  + (window.pageYOffset || options.trigger.scrollTop )  - ( options.trigger.clientTop  || 0 );
        var triggerElementBottom = triggerElementTop + options.trigger.offsetHeight;
